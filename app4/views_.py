...
# (1) Categories Views
#___________________________________________
def category_function_read_all(request):
    categoryVariable_read_all_result = CategoryModel.all()
    print(categoryVariable_read_all_result)
    return render(
        request
        ,'categories/categoryPage_read_all.html'
        ,{
        'categoryVariable_read_all_result'
        : categoryVariable_read_all_result
        }
        )
#___________________________________________
def category_function_read_detail(request, categoryParameter_pk):
    categoryVariable_read_detail_result = CategoryModel.read(categoryParameter_pk)
    return render(
        request,
        'categories/categoryPage_read_detail.html'
        ,{
        'categoryVariable_read_detail_result'
        : categoryVariable_read_detail_result
        }
        )
#___________________________________________
def category_function_create(request):
    if request.method == 'POST':
        ...
        return redirect('category_url_read_all')
    return render(
        request
        ,'categories/categoryPage_create.html'
        )
#___________________________________________
def category_function_update(request, categoryParameter_pk):
    pass
    # if request.method == 'POST':
    #     categoryVariable_update_result = CategoryModel.update(
    #     id=categoryParameter_pk
    #     ,categoryField_name='fayiez'
    #     ,categoryField_slug='fayiez'
    #     ,categoryField_image=None
    #     ,categoryField_available=True
    #     )
    #     print("Category updated")
    #     return redirect('categoryURLs_read_detail')
    # print("Category No updated")
    # return render(
    #     request,
    #     'categories/categoryPage_update.html'
    #     ,{
    #     'categoryVariable_update_result':
    #     categoryVariable_update_result
    #     }
    #     )
#___________________________________________
def category_function_delete(request, parameter_pk):
    CategoryModel.delete(id=parameter_pk)

    # categoryVariable_delete_process = \
    #     get_object_or_404(
    #         CategoryModel
    #         ,pk=parameter_pk
    #                     )
    # categoryVariable_delete_process.delete()
    print("Category Delete OK")
    return redirect('categoryURLs_read_all')
#___________________________________________
def category_function_search(request):
    queryVariable_query = \
        request.GET.get('query')
    categoryVariable_search_result = \
        CategoryModel.search(
        name__icontains=queryVariable_query
                            )
    return render(
        request,
        'categories/categoryPage_list.html'
        ,
        {'categoryVariable_search_result':
        categoryVariable_search_result
        }
