...
class ModelCategoryAdmin(admin.ModelAdmin):
	# The fields to be shown on the Admin page
	list_display        = ('picture_displayDEF','category_name',
							'category_availability','category_image',)
...
# The comments `#______________________________________________________` are used as section
# separators or markers to visually divide different sections of the code related to the Product
# Model. In this case, it helps in organizing the code and making it easier to identify which part of
# the code corresponds to the Product Model. It serves as a visual indicator for developers to quickly
# navigate through the code and understand the structure.
#______________________________________________________
#(03)PRODUCT-MODEL:
#______________________________________________________
...
class ModelProductAdmin(admin.ModelAdmin):
	# The fields to be shown on the Admin page
	list_display        = ('picture_displayDEF','product_name',
							'product_category','product_price','product_stock',
							'product_description','product_availability',
							'product_image',)
	# search_fields        = ('product_name',)
	search_fields        = ('product_category',)
	# search_fields        = ('product_price',)
	...
	list_filter          = ('product_category','product_name',
							'product_category','product_price','product_stock','product_availability',) # filter by Available Field
...
#(04)ORDER-MODEL:
#______________________________________________________
# Add The Child Table(ModelCart) Inside The Parent Table(ModelOrder) 
class ModelCartAdmin(admin.TabularInline):
	model = ModelCart
...
class ModelOrderAdmin(admin.ModelAdmin):
	...
	list_display        = ('order_number','order_date',
							'order_customer','order_status','order_is_finished',)
	# The line `list_display =
	# ('picture_displayDEF','product_name','category_available','category_image',)` in the
	# `ModelProductAdmin` class is defining the fields that will be displayed in the admin interface for
	# the `ModelProduct` model.
	list_filter          = ('order_date','order_number',
							'order_status','order_is_finished','order_customer',) # filter by Available Field
	# list_editable        = ('order_customer','order_status','order_is_finished',)
	# prepopulated_fields  = {'product_slug':('product_name',)} # Auto Filled # Autofill Slug field from name field  
	empty_value_display = '-empty-'
	inlines = [ModelCartAdmin]
...
class ModelCartAdmin(admin.ModelAdmin):
	# The fields to be shown on the Admin page
	list_display        = ('picture_displayDEF','cart_product',
							'cart_order','cart_quantity','cart_price','cart_creation_date',)
	# The line `list_display =
	# ('picture_displayDEF','product_name','category_available','category_image',)` in the
	# `ModelProductAdmin` class is defining the fields that will be displayed in the admin interface for
	# the `ModelProduct` model.
...
#______________________________________________________
# Admin View for Category Model
class ModelCouponAdmin(admin.ModelAdmin):
	# The fields to be shown on the Admin page
	list_display        = ('coupon_code','valid_from',
							'valid_to','discount','discount','amount','active',)
	# The line `list_display =
	# ('picture_displayDEF','product_name','category_available','category_image',)` in the
	# `ModelProductAdmin` class is defining the fields that will be displayed in the admin interface for
	# the `ModelProduct` model.
	list_filter          = ('valid_from','valid_to','discount','discount','active',) # filter by Available Field
	# list_editable        = ('cart_quantity','cart_price',)
	# prepopulated_fields  = {'product_slug':('product_name',)} # Auto Filled # Autofill Slug field from name field  
	empty_value_display = '-empty-'

# Display the Model on the Admin Page
admin.site.register(ModelCoupon, ModelCouponAdmin)
